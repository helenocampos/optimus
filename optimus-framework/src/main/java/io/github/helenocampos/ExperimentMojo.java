        ...
        extends OptimusMojo
{
    ...
    {
        if (!this.getDbPath().equals(""))
        {
            File originalDB = new File(this.getDbPath());
            if (originalDB.exists() && !originalDB.isDirectory())
            {
                File backupDB = new File(this.getDbPath() + ".backup");
                try
                {
    ...
    private void deleteDBBackup()
    {
        if (!this.getDbPath().equals(""))
        {
            File backupDB = new File(this.getDbPath() + ".backup");
            if (backupDB.exists() && !backupDB.isDirectory())
            {
                try
                {
                    FileDeleteStrategy.FORCE.delete(backupDB);
                } catch (IOException ex)
                {
                    Logger.getLogger(ExperimentMojo.class.getName()).log(Level.SEVERE, null, ex);
                }
    ...
    private void executeVersionsExperiment() throws MojoExecutionException
    {
        if (this.getVersionsFolder() != null)
        {
            ...
            boolean firstVersionExecution = true;
    ...
    private void executeTechniques(File projectFolder, boolean generateFaultsFile, boolean firstVersionExecution) throws MojoExecutionException
    {
        ...
        if (this.getPrioritizationTechniques() != null)
        {
            ...
            {
                invokePrioritization(technique, projectFolder, generateFaultsFile, true, false, false, simulateExecution, firstVersionExecution);
            }
        } else
        {
            ...
            {
                invokePrioritization(this.getPrioritization(), projectFolder, generateFaultsFile, true, false, false, simulateExecution, firstVersionExecution);
            }
    ...
    {
        invokePrioritization("default", projectFolder, true, false, true, true, false,false);
    }
    ...
    private void invokePrioritization(String technique, File outputExperimentFolder, boolean generateFaultsFile, boolean calcAPFD, boolean collectCoverage, boolean cleanProject, boolean simulateExecution, boolean firstVersionExecution) throws MojoExecutionException
    {
        ...
        for (int i = 0; i < executionsAmount; i++)
        {
            ...
            pomManager.setupPrioritizationPlugin(getPrioritizationConfig(technique, outputExperimentFolder.getAbsolutePath(), generateFaultsFile, calcAPFD, collectCoverage, simulateExecution, firstVersionExecution));
    ...
    private PrioritizationConfig getPrioritizationConfig(String technique, String projectFolder, boolean generateFaultsFile, boolean calcAPFD, boolean collectCoverage, boolean simulateExecution, boolean firstVersionExecution)
    {
        ...
        config.setFirstVersionExecution(firstVersionExecution);
        return config;
    }
