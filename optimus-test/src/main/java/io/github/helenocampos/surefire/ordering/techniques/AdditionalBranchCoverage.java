...
public class AdditionalBranchCoverage extends AdditionalCoverageOrderer<AbstractTest>
{
    ...
    public AdditionalBranchCoverage()
    {
        this.analyzer = new CoverageAnalyzer();
        this.setCoveredCode(analyzer.getCoveredBranches());
        this.loopLock = false;
    }
    ...
    //coveredCode has branches that are already covered by selected tests
    @Override
    public AbstractTest getHighestCoverageTest(List<AbstractTest> tests)
    {
        ...
        //save initial state of covered branches before selecting the highest coverage test
        HashMap<String, boolean[]> coveredCode = CoverageAnalyzer.copyCoverageData(analyzer.getCoveredBranches());

        //initialize highest test coverage with already selected tests coverage
        HashMap<String, boolean[]> coveredCodeByHighestScoreTest = CoverageAnalyzer.copyCoverageData(analyzer.getCoveredBranches());

        for (AbstractTest test : tests)
        {
            float testScore = analyzer.getTestScore(test, "branch", "additional");
            if (testScore > highestCoverageScore)
            {
                ...
                coveredCodeByHighestScoreTest.putAll(analyzer.getTestCoverage(test, "branch"));
                highestCoverageTest = test;
                highestCoverageScore = testScore;
            }

            // return coveredbranches state to saved state before selecting 
            analyzer.setCoveredBranches(CoverageAnalyzer.copyCoverageData(coveredCode));
        ...
        if (highestCoverageTest != null)
        {
            ...
            this.analyzer.setCoveredBranches(coveredCode);
        ...
        { // no tests in the list add any coverage to already covered code by already selected tests
            ...
            HashMap<String, boolean[]> clearBranchCoverage = this.analyzer.getClearBranchCoverage();
            this.setCoveredCode(clearBranchCoverage);
            this.analyzer.setCoveredBranches(clearBranchCoverage);
            loopLock = true;
